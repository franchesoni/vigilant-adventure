...
class SAMPaper:
    ...
    def generate(self, img):
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            ...
            masks = masks if self.logits else self.predictor.model.mask_threshold < masks
        ...
        boxes = batched_mask_to_box(self.predictor.model.mask_threshold < masks if self.logits else masks)
...
class MSSam(SAMPaper):
    def __init__(self, *args, pca_dim=256, pamss_path="pamms/build/pamss", **kwargs):
        assert kwargs['logits'] and kwargs['box_nms_thresh'] > 0.9, "MSSam requires logits and box_nms_thresh > 0.9"
    ...
    def generate(self, img):
        ...
        pcafeats = pcafeats.transpose(2, 0, 1).astype(np.float32)  # convert to format expected by MS
        ...
        n_masks = [self.max_proposals // (2**i) for i in range(1, int(np.floor(np.log2(self.max_proposals))))]
        for n in n_masks:
            call_command = f"./{self.pamss_path} -n {n} tmp.npy tmp.png labels_{n}.png"
...
def tryit(imgpath, scales=[0, 1, 2], points_per_batch=256, use_cpu=False, box_nms_threshold=0.9, logits=False):
    ...
    mask_generator = MSSam(
        sam, points_per_batch=points_per_batch, scales=scales, box_nms_thresh=box_nms_threshold, use_cpu=use_cpu, logits=logits
    )
    masks = mask_generator.generate(img)
    # H, W = masks[0]["segmentation"].shape
    # C = len(masks)
    # print("masks shape: ", C, H, W)
